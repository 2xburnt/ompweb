...
async function allowedCwd(cwd: unknown): Promise<string> {
  ...
  if (!(await isExistingFilePathAllowed(cwd, allowedRoots))) throw new Error("Workspace is not allowed");
  return cwd;
}
...
export const GET = withHostRoute(async (request: NextRequest) => {
  try {
    ...
    const file = cwd ? await readMcpConfig(cwd, host) : null;
    const user = await readUserMcpConfig(undefined, host);
    const inventory: McpLiveServer[] = [
      ...
      ...(await readDiscoveredMcpServers(cwd ?? undefined, user.disabledServers, host)),
    ];
    ...
    return NextResponse.json({ root: file?.root ?? null, path: file?.path ?? null, exists: file?.exists ?? false, servers: Object.entries(file?.config.mcpServers ?? {}).sort(([a], [b]) => a.localeCompare(b)).map(([name, config]) => ({ name, config: redactMcpServer(config) })), user: safeUser, inventory, liveServers, liveError, host: host.id });
  } catch (error) {
...
export const POST = withHostRoute(async (request: NextRequest) => {
  try {
    ...
    return NextResponse.json({ success: true, ...(await writeMcpServer(cwd, body.name as string, body.server, body.previousName, host)), host: host.id });
  } catch (error) {
    return mcpErrorResponse(error);
  }
});
...
export const DELETE = withHostRoute(async (request: NextRequest) => {
  try {
    ...
    return NextResponse.json({ success: true, ...(await deleteMcpServer(cwd, body.name, host)), host: host.id });
  } catch (error) {
    return mcpErrorResponse(error);
  }
});
