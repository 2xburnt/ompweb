...
// by realpath confinement on the host (resolveSubagentArtifact).
 ...
 * session's sibling artifacts dir on the session's host. Mirrors the RPC
 * get_subagent_messages response shape so the dialog can fall back to it
 * when no live RPC process knows the file.
 *
...
export const GET = withSessionRoute(async (
  ...
  try {
    ...
    if (searchParams.get("mode") === "completion") {
      const resolved = await resolveSubagentArtifact(filePath, subagentId, ".md");
      if (!resolved) {
      ...
      const completion = await readCompletionArtifact(resolved);
    ...
    const resolved = await resolveSubagentArtifact(filePath, subagentId, ".jsonl");
    if (!resolved) {
    ...
    const page = await readSubagentTranscriptPage(resolved, fromByte);
