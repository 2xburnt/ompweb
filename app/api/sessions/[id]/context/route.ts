...
export const GET = withSessionRoute(async (
  ...
  try {
    ...
    if ("response" in resolved) {
      ...
      if (session?.isAlive() && !await currentHost().fs.exists(session.sessionFile)) {
    ...
    if (boundary === "1") {
      return NextResponse.json(await getSessionContextBoundary(filePath), { headers: { "Cache-Control": "no-store" } });
    }
    if (sync === "1") {
