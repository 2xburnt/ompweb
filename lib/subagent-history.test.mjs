...
test("extracts subagent roster from task toolResults with settled results winning", async () => {
  const { dir, sessionFile } = makeSessionFixture();
  try {
    const roster = await extractSubagentHistory(sessionFile);
...
test("keeps async-only spawns (empty results) as started entries", async () => {
  const dir = mkdtempSync(join(tmpdir(), "ompweb-subagent-history-"));
...
test("pages subagent transcripts byte-wise with UI messages", async () => {
  const { dir, sessionFile } = makeSessionFixture();
  try {
    const transcriptFile = join(siblingDirForSession(sessionFile), "ScoutAgent.jsonl");
    const page1 = await readSubagentTranscriptPage(transcriptFile, 0);
    ...
    const page2 = await readSubagentTranscriptPage(transcriptFile, page1.nextByte);
    assert.equal(page2.messages.length, 0);
    assert.equal(page2.nextByte, page1.nextByte);

    // Past EOF resets to the start.
    const page3 = await readSubagentTranscriptPage(transcriptFile, 999999);
...
test("byte-window paging survives non-ASCII content before the offset", async () => {
  const { dir, sessionFile } = makeSessionFixture();
  try {
    const transcriptFile = join(siblingDirForSession(sessionFile), "ScoutAgent.jsonl");
    ...
    const page = await readSubagentTranscriptPage(transcriptFile, fromByte);
...
test("page without a trailing newline still advances (no infinite paging)", async () => {
  const { dir, sessionFile } = makeSessionFixture();
  try {
    const transcriptFile = join(siblingDirForSession(sessionFile), "ScoutAgent.jsonl");
...
test("missing transcript file yields an empty page", async () => {
  const dir = mkdtempSync(join(tmpdir(), "ompweb-subagent-history-"));
  try {
    const page = await readSubagentTranscriptPage(join(dir, "missing.jsonl"), 0);
...
test("completion reads keep complete trailing multibyte characters", async () => {
  ...
  try {
    ...
    assert.equal((await readCompletionArtifact(await resolveSubagentArtifact(sessionFile, "Scout", ".md")))?.completion, "hello");
    writeFileSync(join(artifactsDir, "Scout.md"), "oké");
    assert.equal((await readCompletionArtifact(await resolveSubagentArtifact(sessionFile, "Scout", ".md")))?.completion, "oké");
    writeFileSync(join(artifactsDir, "Scout.md"), "done😀");
    assert.equal((await readCompletionArtifact(await resolveSubagentArtifact(sessionFile, "Scout", ".md")))?.completion, "done😀");
    // Missing file -> unresolvable; empty file -> null.
    assert.equal(await resolveSubagentArtifact(sessionFile, "Nope", ".md"), null);
    writeFileSync(join(artifactsDir, "Scout.md"), "");
    assert.equal(await readCompletionArtifact(await resolveSubagentArtifact(sessionFile, "Scout", ".md")), null);
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});

test("completion caps materialized bytes without splitting a codepoint", async () => {
  const dir = mkdtempSync(join(tmpdir(), "ompweb-subagent-completion-"));
  const sessionFile = join(dir, "sess.jsonl");
  const artifactsDir = siblingDirForSession(sessionFile);
  mkdirSync(artifactsDir, { recursive: true });
  try {
    ...
    const result = await readCompletionArtifact(await resolveSubagentArtifact(sessionFile, "Big", ".md"));
...
test("separate task calls keep their agents apart instead of interleaving", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      (await extractSubagentHistory(sessionFile)).map((entry) => entry.id),
...
test("parallel calls follow the order the assistant issued them, not the order they finished", async () => {
  const dir = mkdtempSync(join(tmpdir(), "ompweb-subagent-order-"));
...
test("an agent seen only in results keeps its own call's place", async () => {
  const dir = mkdtempSync(join(tmpdir(), "ompweb-subagent-order-"));
