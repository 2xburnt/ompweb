import { closeSync, fstatSync, openSync, readSync, statSync, type Stats } from "fs";
...
import { selectHistoryRange, selectSessionHistory, type SessionHistoryCursor, type SessionHistoryPage } from "./session-sync";
...
function sessionCacheKey(filePath: string, host: Host = currentHost()): string {
  const pathKey = sessionPathKey(filePath);
  return host.isLocal ? pathKey : `${host.id}\0${pathKey}`;
}

export function invalidateSessionEntriesCache(filePath: string): void {
  const pathKey = sessionCacheKey(filePath);
  globalThis.__ompSessionEntriesCache?.delete(pathKey);
  for (const [key, index] of globalThis.__ompSessionHistoryIndexes ?? []) {
    if (index.pathKey === pathKey) globalThis.__ompSessionHistoryIndexes?.delete(key);
  }
...
async function loadSessionEntriesCached(filePath: string, host: Host): Promise<SessionEntry[]> {
  ...
  const cacheKey = sessionCacheKey(filePath, host);
  ...
  // Files larger than the entire cache budget are parsed but never retained.
...
function loadSessionEntriesCachedSync(filePath: string): SessionEntry[] {
  let stat: Stats;
  try {
    stat = statSync(filePath);
  } catch {
    return loadSessionFileSync(filePath).entries;
  }
  const cache = getSessionEntriesCache();
  ...
  const entries = loadSessionFileSync(filePath).entries;
  if (stat.size > MAX_SESSION_ENTRIES_CACHE_BYTES) {
    cache.delete(key);
    return entries;
  }
  cache.set(key, { size: stat.size, mtimeMs: stat.mtimeMs, entries });
  let totalBytes = 0;
  for (const entry of cache.values()) totalBytes += entry.size;
  while (cache.size > 1 && (cache.size > MAX_SESSION_ENTRIES_CACHE_ENTRIES || totalBytes > MAX_SESSION_ENTRIES_CACHE_BYTES)) {
    const oldestKey = cache.keys().next().value;
...
export function getSessionEntries(filePath: string, host: Host = currentHost()): SessionEntry[] | Promise<SessionEntry[]> {
  return host.isLocal ? loadSessionEntriesCachedSync(filePath) : loadSessionEntriesCached(filePath, host);
}
...
function loadEntriesOrThrowTooLarge(filePath: string): SessionEntry[] {
  try {
    if (statSync(filePath).size > MAX_SESSION_LOAD_BYTES) {
      ...
      getSessionEntriesCache().delete(sessionCacheKey(filePath));
  ...
  return loadSessionEntriesCachedSync(filePath);
}
...
function toDisplayEntries(entries: SessionEntry[], options: ResolveBlobOptions): SessionEntry[] {
  ...
  if (copiedAny) {
    resolveBlobRefsInEntriesSync(out, options);
  }
  return out;
}
...
async function toDisplayEntriesAsync(
  entries: SessionEntry[],
  ...
  if (copiedAny) await resolveBlobRefsInEntries(out, options, host);
  return out;
}

/** Local deduplicated display read; bookkeeping is visible in the same tick. */
function getLocalSessionEntriesForDisplayAsync(
  filePath: string,
  options: ResolveBlobOptions,
): Promise<SessionEntry[]> {
  const key = sessionCacheKey(filePath);
  ...
  if (stat && stat.size > MAX_SESSION_LOAD_BYTES) {
    cache.delete(key);
    return Promise.reject(new SessionFileTooLargeError(filePath));
  }
  if (stat) {
    const cached = cache.get(key);
    if (cached && cached.size === stat.size && cached.mtimeMs === stat.mtimeMs) {
      cache.delete(key);
      cache.set(key, cached);
      return Promise.resolve(toDisplayEntries(cached.entries, options));
    }
  }
  const inFlight = getInFlightEntries().get(key);
  if (inFlight) return inFlight.then((entries) => toDisplayEntries(entries, options));
  const parse = Promise.resolve().then(() => loadSessionEntriesCachedSync(filePath));
  getInFlightEntries().set(key, parse);
  return parse.then((entries) => toDisplayEntries(entries, options)).finally(() => {
    const map = getInFlightEntries();
    if (map.get(key) === parse) map.delete(key);
  });
}

async function getRemoteSessionEntriesForDisplayAsync(
  filePath: string,
  options: ResolveBlobOptions,
  host: Host,
): Promise<SessionEntry[]> {
  const key = sessionCacheKey(filePath, host);
  const cache = getSessionEntriesCache();
  ...
  if (stat) {
    ...
    if (cached && cached.size === stat.size && cached.mtimeMs === stat.mtimeMs) {
      cache.delete(key);
      cache.set(key, cached);
      return toDisplayEntriesAsync(cached.entries, options, host);
    }
  }
  const inFlight = getInFlightEntries().get(key);
  if (inFlight) return toDisplayEntriesAsync(await inFlight, options, host);
  const parse = loadSessionEntriesCached(filePath, host);
  getInFlightEntries().set(key, parse);
  try {
    return await toDisplayEntriesAsync(await parse, options, host);
  } finally {
    const map = getInFlightEntries();
    if (map.get(key) === parse) map.delete(key);
  }
...
/** Host-aware deduplicated display read. */
export function getSessionEntriesForDisplayAsync(
  filePath: string,
  options: ResolveBlobOptions = {},
): Promise<SessionEntry[]> {
  const host = currentHost();
  return host.isLocal
    ? getLocalSessionEntriesForDisplayAsync(filePath, options)
    : getRemoteSessionEntriesForDisplayAsync(filePath, options, host);
}
...
function buildHistoryIndex(
  ...
  const loaded = loadSessionFileSync(filePath, {
  ...
  return { ...detached, pathKey: sessionCacheKey(filePath), version, bytes, entryIds, positions };
}
...
export function getSessionContextBoundary(filePath: string): { entryIds: string[] } | Promise<{ entryIds: string[] }> {
  const host = currentHost();
  if (!host.isLocal) {
    return Promise.resolve(getSessionEntries(filePath, host)).then((entries) => ({
      entryIds: selectSessionContext(entries).selected.map((entry) => entry.id),
    }));
  }
...
function getLocalSessionHistoryPage(
  filePath: string,
...
type SessionHistoryResult = { history: SessionHistoryPage; leafId: string | null; tipId: string | null };

async function getRemoteSessionHistoryPage(
  filePath: string,
  cursor: SessionHistoryCursor | null,
  limit: number,
  leafId?: string,
  ...
  const entries = await getSessionEntriesForDisplayAsync(filePath, {
    skipToolResultImages: options.deferToolResultImages,
  });
  const tipId = entries.at(-1)?.id ?? null;
  const resolvedLeafId = leafId && entries.some((entry) => entry.id === leafId) ? leafId : tipId;
  const context = buildSessionContext(entries, resolvedLeafId, options);
  return {
    history: selectSessionHistory(context, cursor, limit),
...
export function getSessionHistoryPage(
  ...
  leafId?: string,
  options: { deferThinking?: boolean; deferToolResultImages?: boolean; includePreCompaction?: boolean } = {},
): SessionHistoryResult | Promise<SessionHistoryResult> {
  return currentHost().isLocal
    ? getLocalSessionHistoryPage(filePath, cursor, limit, leafId, options)
    : getRemoteSessionHistoryPage(filePath, cursor, limit, leafId, options);
}
