...
test("reads only a bounded session header, including headers larger than 4 KiB", async () => {
  ...
  try {
    assert.equal((await readSessionHeader(filePath))?.parentSession, parentSession);
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});

test("returns null for malformed or unbounded session headers", async () => {
  ...
  try {
    assert.equal(await readSessionHeader(malformedPath), null);
    assert.equal(await readSessionHeader(oversizedPath), null);
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});
...
test("stops resolving a session path once the file is deleted", async () => {
  await withAgentDir(async (dir) => {
    ...
    // --resume against a missing file, silently creating a new session — the
    // spawn paths therefore ask for a verified resolution.
    assert.equal(await resolveSessionPath("doomed", { verify: true }), null);
    assert.equal(globalThis.__piSessionPathCache?.has("doomed"), false);
  });
});

test("loads sessions larger than the line-reader chunk without corrupting text", async () => {
