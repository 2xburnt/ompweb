...
function inFreshDocument(storage, action, setup = "") {
  const script = `
    ...
    ${setup}
    const { setDraft, getDraft, getDraftSummary, clearDraft, hasUnrestorableDrafts } =
...
test("only text survives navigation, including when an attachment-only draft replaces it", () => {
  const saved = inFreshDocument([], `
    ...
    const result = { attachments: getDraftSummary("attachments").hasAttachments, unrestorable: hasUnrestorableDrafts() };
  `);
  assert.deepEqual(saved.result, { attachments: true, unrestorable: true });
  const restored = inFreshDocument(saved.storage, `
    const result = { draft: getDraft("attachments"), unrestorable: hasUnrestorableDrafts() };
    setDraft("attachments", {
      value: "",
      images: [{ data: "data:image/png;base64,AAAA", mimeType: "image/png" }],
      files: [],
  ...
  `);
  // Nothing survives that a reload could lose, so nothing should prompt on one.
  assert.deepEqual(restored.result.draft, { value: "Keep text", images: [], files: [] });
  assert.equal(restored.result.unrestorable, false);
  const cleared = inFreshDocument(restored.storage, `
