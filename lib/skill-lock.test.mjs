...
test("annotates only lock entries that exist in the matching Pi scope", async () => {
  ...
  try {
    ...
    const annotated = await annotateSkillsWithInstallInfo(
      [
...
test("ignores stale lock entries and malformed lock files", async () => {
  const root = mkdtempSync(join(tmpdir(), "pi-web-skill-lock-"));
  try {
    const cwd = join(root, "project");
    ...
    const skills = await annotateSkillsWithInstallInfo(
      [
...
test("does not compare a project ref with the default skills.sh snapshot", async () => {
  const root = mkdtempSync(join(tmpdir(), "pi-web-skill-lock-"));
  try {
    const cwd = join(root, "project");
    const agentDir = join(root, "agent");
    ...
    const [skill] = await annotateSkillsWithInstallInfo(
