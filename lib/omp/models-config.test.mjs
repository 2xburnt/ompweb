...
async function withAgentDir(run) {
  ...
  try {
    await run(dir, join(dir, "models.yml"));
  } finally {
...
test("round-trips a hand-edited models.yml without losing comments", async () => {
  await withAgentDir(async (dir, path) => {
    mkdirSync(dir, { recursive: true });
    writeFileSync(path, HAND_EDITED, "utf8");

    const file = await readModelsConfigFile();
    ...
    await writeModelsConfig(file.config);
    ...
    assert.equal((await readModelsConfigFile()).config.providers["local-llama"].models[0].maxTokens, 16384);
  });
});

test("a save with no edits leaves the file byte-identical", async () => {
  await withAgentDir(async (dir, path) => {
    mkdirSync(dir, { recursive: true });
    writeFileSync(path, HAND_EDITED, "utf8");
    await writeModelsConfig((await readModelsConfigFile()).config);
    assert.equal(readFileSync(path, "utf8"), HAND_EDITED);
  });
});

test("keeps a model's comments with the model when siblings are removed", async () => {
  await withAgentDir(async (dir, path) => {
    mkdirSync(dir, { recursive: true });
    writeFileSync(path, HAND_EDITED, "utf8");

    const { config } = await readModelsConfigFile();
...
test("adds and removes providers", async () => {
  await withAgentDir(async (dir, path) => {
    mkdirSync(dir, { recursive: true });
    writeFileSync(path, HAND_EDITED, "utf8");
    ...
    const reread = await readModelsConfigFile();
...
test("reports a parse error instead of an empty config", async () => {
  await withAgentDir(async (dir, path) => {
    mkdirSync(dir, { recursive: true });
...
test("refuses to overwrite an unparseable models.yml", async () => {
  await withAgentDir(async (dir, path) => {
    mkdirSync(dir, { recursive: true });
    ...
    await writeModelsConfig({ providers: { a: { baseUrl: "https://x/v1", apiKey: "K", api: "openai-completions" } } }, { overwriteUnparseable: true });
    assert.deepEqual(Object.keys((await readModelsConfigFile()).config.providers), ["a"]);
  });
});

test("treats a non-mapping models.yml as unparseable", async () => {
  await withAgentDir(async (dir, path) => {
    mkdirSync(dir, { recursive: true });
    writeFileSync(path, "- one\n- two\n", "utf8");
    assert.ok((await readModelsConfigFile()).parseError);
  });
});

test("writes a fresh file when none exists", async () => {
  await withAgentDir(async (dir, path) => {
    const file = await readModelsConfigFile();
    assert.equal(file.exists, false);
    await writeModelsConfig({ providers: { p: { baseUrl: "https://x/v1", apiKey: "K", api: "openai-completions", models: [{ id: "m" }] } } });
...
test("ignores blank model rows while preserving non-empty rows", async () => {
  await withAgentDir(async (dir, path) => {
    mkdirSync(dir, { recursive: true });
    writeFileSync(path, "providers:\n  p:\n    models:\n      - id: \"\"\n      - id: \"   \"\n      - id: valid\n", "utf8");
    ...
    await writeModelsConfig(file.config);
    assert.deepEqual((await readModelsConfigFile()).config.providers.p.models.map((model) => model.id), ["valid"]);
  });
});
