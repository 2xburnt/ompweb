...
test("resolveSpawnCwd uses the recorded directory when it exists, falls back otherwise", async () => {
  ...
  assert.equal(await resolveSpawnCwd(live), live);
  assert.deepEqual(await resolveSpawnCwdResult(live), { cwd: live, fellBack: false });

  // A missing recorded cwd falls back to a live directory and reports it.
  const missing = "/nonexistent/path/that/should/not/exist";
  const result = await resolveSpawnCwdResult(missing);
  ...
  assert.equal(await resolveSpawnCwd(missing), process.cwd());

  // undefined/empty also falls back.
  assert.equal((await resolveSpawnCwdResult(undefined)).fellBack, true);
  assert.equal(await resolveSpawnCwd(undefined), process.cwd());
});
