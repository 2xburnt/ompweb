...
test("removing a machine takes its workspaces, sessions and tabs with it", async () => {
  const sidebar = await readFile(new URL("../components/SessionSidebar.tsx", import.meta.url), "utf8");
  const appShell = await readFile(new URL("../components/AppShell.tsx", import.meta.url), "utf8");
  // Per-machine maps are pruned to the machines that still exist, otherwise a
  // removed machine's rows linger with no way to open or clean them up.
  assert.match(sidebar, /setProjectsByHost\(pruneByHost\)/);
  assert.match(sidebar, /setHomeByHost\(pruneByHost\)/);
  assert.match(sidebar, /setProjectsErrorByHost\(pruneByHost\)/);
  assert.match(sidebar, /setWorktreeStateByProject\(pruneByHostPrefix\)/);
  // A selection left on a removed machine keeps the explorer pointed at it.
  assert.match(sidebar, /if \(activeHostRef\.current && !live\.has\(activeHostRef\.current\)\)/);
  // Open file tabs on a removed machine can no longer be read.
  assert.match(appShell, /previous\.filter\(\(tab\) => !tab\.hostId \|\| live\.has\(tab\.hostId\)\)/);
});

test("the settings machine pill is a switcher when there is somewhere to switch to", async () => {
  const note = await readFile(new URL("../components/MachineScopeNote.tsx", import.meta.url), "utf8");
  assert.match(note, /MachineMenuList/);
  assert.match(note, /aria-haspopup="menu"/);
  // With a single machine there is nothing to pick, so it stays a label.
  assert.match(note, /const canSwitch = hosts\.filter\(\(host\) => host\.enabled\)\.length > 1;/);
  assert.match(note, /if \(!canSwitch\) \{\s*return <div role="note"/);
});
