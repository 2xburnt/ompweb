...
test("worktree discovery filters prunable entries and identifies the main checkout", async () => {
  ...
  assert.match(source, /isMain:\s*samePath\([\w.]+,\s*repoRoot\)/);
...
test("OMP update route permits check and restart actions with force support", async () => {
  ...
  assert.match(route, /checkOmpUpdate\(body\.force === true, host\)/);
  ...
  assert.match(appShell, /(?:host)?[fF]etch\("\/api\/omp-update"[\s\S]*?action:\s*"check"/);
...
test("settings groups runtime preferences and resource managers behind tabs", async () => {
  ...
  // Runtime models belong to one machine: the load is pinned to an explicit
  // host id so a machine switch mid-request cannot land on the wrong machine.
  assert.match(models, /hostFetch\("\/api\/models", \{ cache: "no-store" \}, \w+ \?\? undefined\)/);
...
test("model endpoint invalidates cached runtime models after external config edits", async () => {
  ...
  // The fingerprint is taken through the host boundary (a remote host's
  // models.yml is stat'ed over ssh), never with a local statSync.
  assert.match(route, /host\.fs\.stat\(/);
  assert.doesNotMatch(route, /statSync/);
